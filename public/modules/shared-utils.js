...
function __maskEmail(email) {
  ...
  const maskedLocal = local.length <= 2 ? `${local[0]}${"*".repeat(Math.max(local.length - 1, 0))}` : `${local[0]}${"*".repeat(Math.max(local.length - 2, 1))}${local.slice(-1)}`;
...
function __toAuditJson(value) {
  ...
  if (typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).filter(([, v]) => typeof v !== "function" && v !== void 0).map(([k, v]) => [k, __toAuditJson(v)])
    );
  }
