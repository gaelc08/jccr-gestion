...
export function createExportUI({
  ...
  async function exportTimesheetHTML() {
    ...
    const noPrintDiv = '<div class="no-print" style="margin-bottom:10px;text-align:center"><button class="print-button" onclick="window.print()">\uD83D\uDDB8 Imprimer / Enregistrer en PDF</button><button class="print-button close-button" onclick="window.close()">\u2716 Fermer</button></div>';
    const iframeHtml = html.replace(noPrintDiv, '');
    __showMileagePreviewModal(iframeHtml, `fiche_presence_${String(currentCoach.name || 'intervenant').replace(/[^a-z0-9_\-]/gi, '_')}_${currentMonth}.html`, html);
